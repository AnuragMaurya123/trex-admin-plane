...
export default function DistributorAssignment({ orders, getStatusColor,setOrders}:DistributorAssignmentProps) {
      const [selectedDistributor, setSelectedDistributor] = useState<string>("")
      const [distributorNotes, setDistributorNotes] = useState("")
      const [assignmentSearchTerm, setAssignmentSearchTerm] = useState("")
    // Filter orders for distributor assignment (confirmed orders without distributor)
  const unassignedOrders = orders.filter((order) => {
    const matchesSearch =
      order.orderNumber.toLowerCase().includes(assignmentSearchTerm.toLowerCase()) ||
      order.customerName.toLowerCase().includes(assignmentSearchTerm.toLowerCase())
    const isUnassigned = !order.assignedDistributor && (order.status === "confirmed" || order.status === "processing")
  ...
  return (
    <div> <TabsContent value="assignment" className="space-y-6">
                                    ...
                                    <div className="flex items-center gap-2">
                                      <div>
                                        ...
                                        <div className="text-sm text-slate-600">
                                          {distributor.city} • {distributor.currentOrders}/{distributor.capacity} orders
                                        </div>
                                      </div>
                                    </div>
                                  </SelectItem>
                        ...
                        <CardContent className="p-4">
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <Badge
                              variant="outline"
                              className={
                                distributor.currentOrders / distributor.capacity > 0.8
                                  ? "border-red-300 text-red-700"
                                  : "border-green-300 text-green-700"
                              }
                            >
                              {distributor.currentOrders}/{distributor.capacity}
                            </Badge>
                          </div>
                            ...
                            <div className="w-full bg-gray-200 rounded-full h-2">
                              <div
                                className={`h-2 rounded-full ${
                                  distributor.currentOrders / distributor.capacity > 0.8 ? "bg-red-500" : "bg-green-500"
                                }`}
                                style={{
                                  width: `${(distributor.currentOrders / distributor.capacity) * 100}%`,
                                }}
                              ></div>
                            </div>
                            <p className="text-xs text-slate-500 mt-1">
                              Capacity: {Math.round((distributor.currentOrders / distributor.capacity) * 100)}%
                            </p>
                          </div>
                        </CardContent>
                      </Card>
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="text-blue-700">Order</TableHead>
                        ...
                        <TableCell>
                          {order.assignedDistributor && (
                            <div>
                              ...
                              <div className="text-sm text-slate-600">{order.assignedDistributor.city}</div>
                            </div>
                          )}
                        </TableCell>
                        <TableCell>{order.assignedDate && new Date(order.assignedDate).toLocaleDateString()}</TableCell>
                        <TableCell>
                          <Badge className={getStatusColor(order.status)}>
                            {order.status.charAt(0).toUpperCase() + order.status.slice(1)}
                          </Badge>
